...
const Register = ({ history, registerUser = f => f }) => {
    let _email, _password, _name, _confirm_password;

    const handleLogin = e => {
        e.preventDefault();

        registerUser(_name.value, _email.value, _password.value,  _confirm_password.value);
    };
    return (
        <div id="main">
            <form id="login-form" action="" onSubmit={handleLogin} method="post">
                ...
                />
                <input
                    ref={input => (_confirm_password = input)}
                    style={styles.input}
                    autoComplete="off"
